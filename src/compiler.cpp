...
piranha::IrCompilationUnit *piranha::Compiler::analyze_script(const char *InlineScript)
{
    IrCompilationUnit *newUnit = nullptr;
    newUnit = TRACK(new IrCompilationUnit());
    newUnit->setErrorList(&m_errorList);
    IrCompilationUnit::ParseResult parseResult = newUnit->parse(InlineScript);
...
piranha::IrCompilationUnit *piranha::Compiler::compile_script(const char *InlineScript)
{
    IrCompilationUnit *topLevel = analyze_script(InlineScript);
...
piranha::IrCompilationUnit *piranha::Compiler::getUnit(const IrPath &scriptPath) const
{
    ...
    for (int i = 0; i < nUnits; i++)
    {
        ...
        Path const& UnitPath = unit->getPath();

        if (UnitPath.exists() && UnitPath == scriptPath)
        {
            return unit;
        }
    }
